...
export default function CalibratePage() {
  ...
  const handleSubmit = async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/calibrate/fit", {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        throw new Error(`Server returned ${res.status}: ${err.slice(0, 200)}`);
      }
      const serverProfile = await res.json();
      setProfile(serverProfile);
      // Persist for later /api/compare calls
      try { localStorage.setItem("aqal_calibration", JSON.stringify(serverProfile)); } catch {}
    } catch (e) {
      // Fallback: compute locally if server is down
      console.warn("Server calibration failed, falling back to client fit:", e);
      ...
      const fallback = {
        axis_sensitivity: {
          visual: 1 - (axes.visual / counts.visual - 3) * 0.15,
          auditory: 1 - (axes.auditory / counts.auditory - 3) * 0.15,
          social: 1 - (axes.social / counts.social - 3) * 0.15,
        },
        network_modulation: {
          visual: 1 - (axes.visual / counts.visual - 3) * 0.15,
          motor: 1 - (axes.auditory / counts.auditory - 3) * 0.15,
          default_mode: 1 - (axes.social / counts.social - 3) * 0.12,
          salience: 1 - ((axes.visual + axes.auditory) / (counts.visual + counts.auditory) - 3) * 0.12,
          emotional: 1 - (axes.social / counts.social - 3) * 0.10,
          attention: 1.0, control: 1.0,
        },
        r_squared: 0,
        n_stimuli_rated: Object.keys(ratings).length,
        notes: ["Server calibration unavailable — using client-side fallback fit."],
      };
      setProfile(fallback);
      try { localStorage.setItem("aqal_calibration", JSON.stringify(fallback)); } catch {}
    }
    setLoading(false);
    setStep(7);
  };
          ...
          <div className="max-w-[700px] mx-auto">
            ...
            <div className="flex items-center gap-6 mb-10">
              {[["6", "Scenarios"], ["3", "Axes per scenario"], ["~5", "Minutes"]].map(([n, l]) => (
                <div key={l}>
                  <div className="text-[18px] font-medium text-[var(--heading)] tabular-nums">{n}</div>
                  <div className="text-[10px] text-[var(--muted)] mt-0.5">{l}</div>
                </div>
              ))}
            </div>
            <button onClick={() => setStep(1)} className="text-[13px] px-5 py-2 rounded-full cta-primary font-medium transition">
              Start Calibration
            </button>
          </div>
        </section>
        ...
        <section className="pt-24 pb-16 px-6">
          <div className="max-w-[700px] mx-auto">
            ...
            <div className="flex items-center gap-2 mb-8">
              {STIMULI.map((_, i) => (
                <div key={i} className={`flex-1 h-1 rounded-full transition-all ${i < step ? "bg-[var(--accent)]" : i === step - 1 ? "bg-[var(--accent)]" : "bg-subtle"}`} />
              ))}
            </div>
                      ...
                      <button key={v} onClick={() => setRating(currentStim.id, axis, v)}
                        className={`flex-1 py-2.5 rounded-lg text-[12px] font-medium transition ${
                          currentRating?.[axis] === v
                            ...
                            : "bg-subtle border border-[var(--border)] text-[var(--muted)] hover:text-[var(--heading)] hover:border-[var(--accent)]/30"
                        }`}>
                        {v}
                      </button>
                    ))}
            ...
            <div className="flex items-center gap-3 mt-8">
              {step > 1 && (
                <button onClick={() => setStep(step - 1)} className="text-[13px] px-5 py-2 rounded-full border border-medium text-[var(--muted)] hover:text-[var(--heading)] transition">
                  Back
                </button>
              )}
              {step < 6 ? (
                <button onClick={() => setStep(step + 1)} disabled={!canProceed}
                  className="text-[13px] px-5 py-2 rounded-full cta-primary font-medium disabled:opacity-30 transition">
                  Next
                </button>
              ) : (
        ...
        <section className="pt-24 pb-16 px-6">
          <div className="max-w-[700px] mx-auto">
            <div className="flex items-center gap-2 mb-5">
              ...
              <span className="text-[11px] text-status-green tracking-widest uppercase font-medium">Calibration Complete</span>
            </div>
            ...
            <div className="card p-5 border-l-2 border-[var(--accent)] mb-6">
              <p className="text-[12px] text-[var(--muted)] leading-relaxed font-light">
                <strong className="text-[var(--heading)] font-normal">What this means:</strong> When you use NeuroBrain&apos;s compare tool, your predictions will be modulated by this profile — amplifying divergence in networks where you reported higher sensitivity, and reducing it where you&apos;re more comfortable. This turns a population average into a personal estimate.
              </p>
            </div>

            <div className="bg-[var(--bg)] rounded-lg p-4 border border-[var(--border)] mb-8">
              <p className="text-[10px] text-[var(--muted)] font-light italic">
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => { setStep(0); setRatings({}); setProfile(null); }}
                className="text-[13px] px-5 py-2 rounded-full border border-medium text-[var(--muted)] hover:text-[var(--heading)] transition">
                Retake
              </button>
              <a href="/" className="text-[13px] px-5 py-2 rounded-full cta-primary font-medium transition">
                Use in NeuroBrain
              </a>
            </div>
          </div>
...
function Nav() {
  return (
    <nav className="fixed top-0 w-full z-50 nav-bg backdrop-blur-xl border-b border-[var(--border)]">
...
function Footer() {
  return (
    <footer className="border-t border-[var(--border)] py-5 px-6">
      <div className="max-w-[700px] mx-auto">
        <p className="text-[10px] text-[var(--muted)] mb-4 leading-relaxed font-light">This calibration is a research tool. Sensory profiles are self-reported estimates, not clinical assessments.</p>
        <div className="flex items-center justify-between text-[11px] text-[var(--muted)]">
          <span>NeuroBrain by Leeza Care</span>
          <a href="https://mind.new" className="hover:text-[var(--heading)] transition">mind.new</a>
        </div>
      </div>
    </footer>
  );
